...
// dd($data);
?>
<{{ $data['php'] }} use Rras3k\SebconsoleRoot\facades\ViewData; ViewData::setEntites($data['rras3k']); ?>
            ...
            <div class="§_panel §_w_600">
                <div class="§_content">
                    ...
                    <form id="role" method="POST"
                        action="{!! '{' !!}{ route('{{ $data['this']->routeName_store }}') }}"
                        ...
                        <form id="role" method="POST"
                            ...
                            :liste="ViewData::form_getHiddenValues()" />
                            {!! '@' !!}method('PUT')
                            {!! '@' !!}endif
                            {!! '@' !!}csrf
                            <div class="row">
                                @foreach ($data['this']->champs as $key => $value)
                                    @if ($value['form']['visible'])
                                        @if ($value['link']['enable'])
                                            ...
                                            :value="ViewData::form_getData('{{ $key }}')"
                                            :liste="ViewData::data_getList('{{ $value['link']['table'] }}')"
